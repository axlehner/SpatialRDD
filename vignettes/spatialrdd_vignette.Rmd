...
Within the last years, the use of spatial versions of Regression Discontinuity Designs has increased tremendously in popularity in the social sciences, most importantly Economics and Political Science.
...
```{r}
library(lfe)
summary(felm(education ~ treated | factor(segment15) | 0 | 0, data = points_samp.sf[points_samp.sf$dist2cutoff < 3000, ]), robust = T)
...
Usually regression models are global, meaning that all data are used to fit one single statistical model. Above we saw some methods that went local, or at least semi-local in the case of fixed effects spec where we then still reported one point estimate
A very famous form of local regression is GWR, which lets the regression coefficients be functions of position in space:

$$
y_i = \sum_{ij} \beta_j (lon_i,\ lat_i)\ x_{ij} + \varepsilon_i.
$$
In other words, we just draw a circle around each observation and then estimate the above equation separately each time, where the sample only consists of points that are contained in the respective circle. It is thus a form of sample splitting, or stratification, if you want.
It is very useful to explore spatial heterogeneity, or non-stationarity.
The `spgwr` package is not yet familiar with the new `sf` class of spatial objects in R, we thus have to convert our spatial data frame back to the `sp` class.

```{r, message = F}
library(spgwr)
points_samp.sp <- as(points_samp.sf, "Spatial")
```

In a next step we have to determine the bandwidth of the radius that decides how many observations we include in our GWR. This decision is somewhat arbitrary and there is a large discussion about it in the literature. A safe approach for us is to use a cross-validation approach that chooses the bandwith so that the RMSE prediction error of the model is minimised:

```{r, message = F}
bw <- gwr.sel(education ~ treated, data = points_samp.sp)
```

The resulting bandwith is around 15 kilometres, which seems reasonable for our application.

```{r}

educ.gwr <- gwr(education ~ treated, data = points_samp.sp, bandwidth = bw)
educ.gwr
...
points_samp.sf$gwr_educ <- educ.gwr$SDF$treated1
points_samp.sf$gwr_educ_SE <- educ.gwr$SDF$gwr.e
points_samp.sf$gwr_educ_SE[points_samp.sf$gwr_educ_SE < 0] <- points_samp.sf$gwr_educ_SE[points_samp.sf$gwr_educ_SE < 0] * -1
tm_shape(points_samp.sf) + tm_bubbles(col = "gwr_educ", palette = "RdBu", size = "gwr_educ_SE")
```

Predicted values. For this we could also the `gwr.predict()` from the `GWmodel` package.


```{r}
points_samp.sf$gwr_pred_educ <- educ.gwr$SDF$pred
tm_shape(points_samp.sf) + tm_dots("gwr_pred_educ", size = .1)
tm_shape(points_samp.sf) + tm_dots("gwr_pred_educ", size = .1) + tm_shape(cut_off.sf) + tm_lines()
```
